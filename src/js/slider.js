    ...
    if(slider){
        ...
        arrows.forEach(arrow => {
            ...
            arrow.addEventListener("click", () => {
                //const indent = 2.5 * (Number((getComputedStyle(document.documentElement).fontSize).replace('px', '')) * 10) / 10;
                let firstImgWidth = firstImg.clientWidth + 25;



                //let firstImgWidth = Number((getComputedStyle(firstImg).width).replace('px', '')) + indent;


                console.log(firstImg.clientWidth)
                //console.log(Number((getComputedStyle(firstImg).width).replace('px', '')));

                console.log(arrow)
                console.log(arrow.classList.contains('slider__button-prev'))
